...
class TestValidatePluginJson:
    ...
    def test_valid_version_formats(self):
        ...
        for version in ["1.0.0", "0.1.0", "2.3.4", "1.0.0-alpha", "1.0.0-beta.1"]:
            ...
            result = validate_plugin_json(Path("plugin.json"), content)
            assert not any("セマンティックバージョニング" in w for w in result.warnings), f"Version {version} should be valid"
    ...
    def test_all_path_fields(self):
        """すべてのパスフィールドをテスト"""
        path_fields = ["commands", "agents", "skills", "hooks", "mcpServers", "lspServers", "outputStyles"]
        for field in path_fields:
            ...
            result = validate_plugin_json(Path("plugin.json"), content)
            assert any("./" in w for w in result.warnings), f"Field {field} should trigger path warning"
