...
- Added `requiredMinimumVersion` and `requiredMaximumVersion` managed settings for org admins to pin or cap the Claude Code version
- `/plugin list` now supports `--enabled` and `--disabled` filters
- Added "c to copy" shortcut in `/btw` for quick text copy
- Fixed stdio MCP servers now receive `CLAUDE_CODE_SESSION_ID` on `--resume` sessions
- Fixed `claude -p` hang when subagent output contains large data
- Fixed Bash command failures in certain shell environments
- Fixed hook `if:` condition evaluation edge cases
- Fixed deny rules not properly applying in some permission scenarios
- UI/UX: improved background session updates and description text
