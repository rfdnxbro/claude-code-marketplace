...
tools: Read, Bash(git:*), Bash(printenv:*), Bash(find:*), Bash(grep:*)
model: sonnet
maxTurns: 8
memory: project
effort: low
...
3. スキップパターンが設定されている場合、カンマで分割した各パターンに対して
   `find` コマンドで検索し、マッチしたファイルを変更ファイルリストから除外する。
   パターンに `**/` プレフィックスがない場合は自動的に再帰検索する
   （例: `find . -name "*.test.ts"` として検索）
4. 変更されたファイルの親ディレクトリとプロジェクトルートから、
   チェック対象パターンにマッチするドキュメントファイルを `find` コマンドで検索する
