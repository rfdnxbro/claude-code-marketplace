...
記事は以下のパスに配置されます（`articlesDir` のデフォルトは `content`）：

```text
<articlesDir>/<カテゴリ>/<サブカテゴリ>/<概念名>.md
```
...
outline: |
  導入問題:
    ...
    問い: どのように改善すべきか
  ...
file_path: content/ヒト/組織行動・リーダーシップ/心理的安全性.md
...
プラグインインストール後、以下の設定をカスタマイズできます:

| 設定キー | デフォルト | 説明 |
|---------|-----------|------|
| `articlesDir` | `content` | 記事の出力先ディレクトリ |
| `authorName` | （空） | 記事のメタデータに使用する著者名 |

設定値はプラグインインストール時に入力するか、後からプラグイン設定で変更できます。`articlesDir` を変更すると、`/bbl:brainstorm` で作成する `.bbl-context.yml` の `file_path` に反映されます。
