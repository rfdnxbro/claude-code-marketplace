...
## 【破壊的変更 v2.1.77】Agent ツールの `resume` パラメータ削除

> **v2.1.77以降**: Agent ツールの `resume` パラメータは**削除**されました。

停止したエージェントを再開するには、`SendMessage({to: agentId})` を使用してください。
...
```javascript
// SendMessage でエージェントIDを指定して再開
SendMessage({ to: agentId, message: "継続するタスクの内容" })
```

### `SendMessage` の自動再開挙動（v2.1.77以降）

`SendMessage` が停止済みエージェントに対してメッセージを送信した場合、エラーを返す代わりに**バックグラウンドで自動的にエージェントを再開**します。

- v2.1.77以前: 停止済みエージェントへの `SendMessage` はエラーを返していた
- v2.1.77以降: 停止済みエージェントを自動的にバックグラウンドで再開してメッセージを処理する
