...
| `effort` | No | 思考努力レベル：`low`, `medium`, `high`, `xhigh`, `max`（`xhigh` / `max` は対応モデルでのみ利用可能。非対応モデルでは`high`にフォールバック） |
| `initialPrompt` | No | エージェント起動時に自動送信される最初のプロンプト |
...
**注意 (macOS/Linux ネイティブビルド)**: macOS および Linux のネイティブビルドでは、`Glob` と `Grep` ツールは提供されず、Bash ツール経由で提供される組み込みの `bfs` および `ugrep` コマンドを使用します。Windows および npm インストールビルドでは `Glob` / `Grep` ツールが利用できます。

- `tools: Glob` や `tools: Grep` を指定した場合、ネイティブビルドではこれらのツールが利用できません
- クロスプラットフォームで動作するプラグインを作成する場合は、`Glob`/`Grep` の代わりに `Bash` ツールを使用することを推奨します

### 未知のツール名のみを指定した場合

`tools` に指定したツール名がすべて存在しない/認識されないものだった場合、エージェントは起動に失敗し、認識されないエントリ名を列挙したエラーになります。ツール名のタイプミスがないか確認してください。
...
**理由**: 確実にスキルをロードするために、完全修飾名の使用を推奨します。
