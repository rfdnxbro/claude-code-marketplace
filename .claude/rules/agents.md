...
## Agent ツールの resume パラメータ廃止（v2.1.77・破壊的変更）

v2.1.77 より、Agent ツールの `resume` パラメータは廃止されました。以前このパラメータを使用して停止済みエージェントを再開していた場合は、`SendMessage` に移行してください。

**移行前（v2.1.76 以前）:**

```markdown
// resume パラメータでエージェントを再開（廃止）
...
```markdown
// SendMessage で agentId を指定して再開
SendMessage({ to: agentId, message: "..." })
```

## SendMessage の自動再開機能（v2.1.77以降）

v2.1.77 より、`SendMessage` は停止済みエージェントに対してもバックグラウンドで自動的に再開するようになりました。

- **v2.1.76 以前**: 停止済みエージェントに `SendMessage` を送るとエラーが返っていた
- **v2.1.77 以降**: 停止済みエージェントはバックグラウンドで自動再開され、エラーなくメッセージを受け取れる

この変更により、エージェントの停止状態を明示的に管理する必要がなくなり、マルチエージェントワークフローがより堅牢になりました。
