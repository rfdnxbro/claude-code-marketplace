...
## PreToolUse フックの "allow" 戻り値に関するセキュリティ修正（v2.1.77）

v2.1.77 で、`PreToolUse` フックが `"allow"` を返した場合に `deny` パーミッションルール（エンタープライズ管理設定を含む）を迂回していたバグが修正されました。

- **v2.1.76 以前**: `PreToolUse` フックで `permissionDecision: "allow"` を返すと、`deny` ルールで禁止されたツールでも実行が許可されてしまう問題があった
- **v2.1.77 以降**: `deny` パーミッションルールが正しく優先され、フックの `"allow"` 戻り値で迂回できなくなった

**影響範囲**: `PreToolUse` フックで `permissionDecision: "allow"` を返して意図的に許可処理を行っていたプラグインは、`deny` ルールとの兼ね合いを確認してください。
