...
**`/resume` でのセッション切り替え時の動作（v2.1.79で修正）:**

v2.1.79以降、インタラクティブな `/resume` コマンドでセッションを切り替えた際にも `SessionEnd` フックが正常に発火するようになりました。それ以前のバージョンでは `/resume` によるセッション切り替え時にフックが発火しないバグがありました。`SessionEnd` フックでセッション終了時の後処理（クリーンアップ、ログ記録など）を実装している場合、このバグ修正によって期待通りに動作します。
