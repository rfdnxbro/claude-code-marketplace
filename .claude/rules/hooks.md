...
**v2.1.90での修正**: `PreToolUse` フックが JSON を stdout に出力しつつ exit code 2 で終了した場合、ツール呼び出しが正しくブロックされない不具合が修正されました。v2.1.90以降は、JSON stdout 出力と exit code 2 の組み合わせでも確実にツール実行がブロックされます。
