...
| `PermissionDenied` | autoモードクラシファイアーによる拒否後 | ✓ |
| `UserPromptSubmit` | ユーザープロンプト送信時 | × |
| `Notification` | 通知発行時 | ✓ |
| `Stop` | Claude終了時 | × |
| `StopFailure` | APIエラー（レート制限・認証失敗など）でターンが終了した時 | × |
| `SubagentStart` | サブエージェント起動時 | ✓ |
| `SubagentStop` | サブエージェント終了時 | ✓ |
| `PreCompact` | コンパクト前（exit code 2または`{"decision":"block"}`でブロック可能） | ✓ |
...
| `TaskCreated` | `TaskCreate` でタスクが作成された時 | × |
...
`args`（オプション）: 文字列の配列でコマンド引数を指定。`args`を指定するとシェルを介さずコマンドを直接起動する（exec形式）。パス指定にクォートが不要になる。省略時は引数なしでシェル経由で実行される。

`async`（オプション）: `true`を指定するとフックを非同期で実行し、結果を待たずにツール実行を続行します。

**async フックの stdin**: `async: true` で実行されるフックでも `bash read -r` 等を使用して stdin を正常に受け取れます。
...
`prompt` 型フックはLLMが評価した結果を `{ok: boolean, reason: string}` のJSON形式で出力します。非Stopイベント（`PreToolUse` など）で `ok: false` が返された場合、ツール実行や処理の継続がブロックされます。`reason` フィールドの内容がユーザーへのブロック理由として表示されます。
...
HTTPエンドポイントにJSONをPOSTし、サーバーからのJSONレスポンスを受信・処理するフックタイプ:
...
> **セキュリティ要件**: ヘッダー値での環境変数展開（`${VAR}` 形式）には、`allowedEnvVars` フィールドへの明示的なホワイトリスト登録が必須です。
...
| 複数（OR）カンマ区切り | `Bash,PowerShell` | BashまたはPowerShell |
...
> **注意**: ハイフン（`-`）を含む識別子（例: `mcp__brave-search`、`code-reviewer`）は**完全一致**になります。ハイフン付きMCPサーバーのすべてのツールにマッチするには、`mcp__brave-search__.*` のように正規表現で `__.*` を末尾に付ける必要があります。エージェント名（例: `code-reviewer`）も同様に完全一致です。
...
- `if` フィールドには単一のパーミッションルールのみ指定可能。`&&` や `||`、リスト構文での複数条件の組み合わせはできない。複数条件を適用したい場合は、条件ごとに別のフックハンドラーを定義する

### Bashパターンのマッチング挙動

Bashコマンドに対する `if` パターンのマッチングは、パターンの形と実際に実行されるコマンドの形によって挙動が変わります。先頭の `VAR=value` 形式の変数代入はマッチング前に除去されます。

| `if` パターン | Bashコマンド | フック実行 | 理由 |
|--------------|-------------|:---------:|------|
| `Bash(git *)` | `FOO=bar git push` | する | 先頭の変数代入は除去され、`git push` にマッチ |
| `Bash(git *)` | `npm test && git push` | する | サブコマンドごとにチェックされ、`git push` にマッチ |
| `Bash(rm *)` | `echo $(rm -rf /)` | する | `$()` やバッククォート内のコマンドもチェックされ、`rm -rf /` にマッチ |
| `Bash(rm *)` | `echo $(date)` | しない | `rm *` にマッチするサブコマンドがない |
| `Bash(git push *)` | `echo $(date)` | する | コマンド名以外の要素まで指定したパターンは、`$()`・バッククォート・`$VAR` が含まれる場合、実際の内容に関わらずフックを実行する |

Bashコマンドがパースできない場合、フィルターは fail open（安全側ではなく実行側に倒れる）し、パターンに関わらずフックが実行されます。`if` フィルターはベストエフォートであるため、確実な許可・拒否を強制する用途にはフックではなく [plugin-manifest.md](plugin-manifest.md) の「パーミッション設定」節で説明する `allow`/`ask` によるパーミッション制御を使用してください。
...
| `2` | ブロック | ツール実行を中止、stderrをユーザーに表示 |
| `1, 3+` | 警告 | stderrを表示し、処理を継続 |

exit code 2でブロックされた場合、stderrの内容がユーザーに表示されます。これにより、フックがツール実行をブロックした理由をユーザーに伝えることができます。
...
### PostToolUse フックでのツール出力置き換え

`PostToolUse` フックは `hookSpecificOutput.updatedToolOutput` を返すことで、ツールの出力を置き換えることができます。Bash、Edit、Write、MCPツールなどすべてのツールに対応しています。
...
| フィールド | 型 | 説明 |
|-----------|---|------|
| `updatedToolOutput` | string | ツールの出力を置き換える文字列。すべてのツール（Bash、Edit、Write、MCPツール等）に対応 |

> **注意**: フックスクリプトが **50K文字を超える出力** を返した場合、コンテキストに直接注入される代わりに **ディスクに保存** され、ファイルパスとプレビューのみが提供されます。大量の出力を返すフックの動作に注意してください。
>
> **注意**: `permissionDecision: "ask"` を返しても、`permissions.deny` ルールが設定されている場合は **deny が優先** されます。
>
> **注意**: `PermissionRequest` フックが `updatedInput` を返した場合も、`permissions.deny` ルールによる再チェックが行われ、deny が優先されます。
...
- `disableBypassPermissionsMode` が設定されている場合、`setMode: "bypassPermissions"` は無効になります

### PreToolUseフックの追加コンテキスト

`PreToolUse`フックは、`additionalContext`フィールドを使用してモデルに追加のコンテキストを提供できます。
...
### PreToolUseフックの "defer" パーミッション決定値
...
| `effort.level` | 現在のエフォートレベル（`low` / `medium` / `high` / `xhigh` / `max`） |
| `agent_id` | サブエージェントから実行された場合のエージェントID |
| `agent_type` | `--agent`フラグで起動された場合のエージェントタイプ |

### Stop / SubagentStop イベントの入力
...
| `background_tasks` | バックグラウンドタスクの情報。TODO: 要確認 - 正確な型・スキーマは未調査 |
| `session_crons` | セッション内クロンの情報。TODO: 要確認 - 正確な型・スキーマは未調査 |
...
**`additionalContext` による継続フィードバック:**
...
### PostToolUse / PostToolUseFailure イベントの追加フィールド

`PostToolUse` および `PostToolUseFailure` フックには、追加フィールド `duration_ms` が渡されます:
...
| フィールド | 型 | 説明 |
|-----------|---|------|
| `reloadSkills` | boolean | `true` を返すとスキルディレクトリを再スキャンし、フックがインストールしたスキルを同じセッションで利用可能にする |
| `hookSpecificOutput.sessionTitle` | string | 起動時・再開時にセッションタイトルを設定する |
...
`SessionEnd` フックのタイムアウトは `hook.timeout` の設定に従います。また、環境変数 `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` を設定することで、セッション終了フックのタイムアウト時間をミリ秒単位でカスタマイズできます。
...
マルチエージェントワークフロー環境で、チームメイトエージェントがアイドル状態（待機中）になったときに実行されるフック。
...
セッション中に設定ファイルが変更されたときに実行されるフック。エンタープライズのセキュリティ監査や設定変更のブロックに使用できます。
...
エージェントのworktree分離（`isolation: worktree`）で、新しいgit worktreeが作成されたときに実行されるフック。
...
エージェントのworktree分離（`isolation: worktree`）で、git worktreeが削除されたときに実行されるフック。
...
CLAUDE.mdまたは`.claude/rules/*.md`ファイルがコンテキストに読み込まれたときに実行されるフック。
...
コンパクション実行前に発火するフック。exit code 2または`{"decision":"block"}`を返すことでコンパクションをブロックできます。
...
コンパクション完了後に実行されるフック。コンパクション結果のブロックや変更はできません（ログ・通知用途）。
...
MCPサーバーが構造化入力（フォームフィールドまたはブラウザURL）をリクエストした際、レスポンス送信前に実行されるフック。
...
autoモード（自動モード）のクラシファイアーがツール実行を拒否した後に発火するフック。
...
| フィールド | 型 | 説明 |
|-----------|---|------|
| `hookSpecificOutput.sessionTitle` | string | セッションのタイトルを設定する |
...
モデル切り替え時に発火するフック。モデル切り替えのブロック・確認・注釈付与に使用できる。

TODO: 要確認 — 入力JSONの固有フィールド、出力JSONスキーマ（ブロック・確認の具体的な返り値形式）、対応するhookタイプ、マッチャー対応の有無は現時点で未調査。
...
プラグインの`hooks`フィールドで指定するフック設定ファイル（`hooks/hooks.json`等）へのパスがシンボリックリンクで、リンク先がプラグインディレクトリ外を指している場合、エラーで拒否されます（プラグインディレクトリ外のファイルを読み取れてしまうことを防ぐため）。詳細は[plugin-manifest.md](plugin-manifest.md)の「パスの重要なルール」を参照してください。
