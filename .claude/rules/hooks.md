...
## PreToolUseフックのパーミッション挙動（v2.1.77セキュリティ修正）

**重要（v2.1.77以降）**: `PreToolUse` フックが `"allow"` を返しても、`deny` パーミッションルール（エンタープライズ管理設定を含む）をバイパスしなくなりました。

v2.1.77より前のバージョンでは、`PreToolUse` フックが `permissionDecision: "allow"` を返すと、管理者が設定した `deny` ルールを上書きできる問題がありました。この修正により、フックによる `"allow"` は `deny` ルールより低い優先度になります。

```text
パーミッション優先順位（高い順）:
1. deny ルール（エンタープライズ管理設定）
2. allow ルール
3. PreToolUse フックの permissionDecision
```

**影響**: `PreToolUse` フックで `permissionDecision: "allow"` を返すことで `deny` ルールを回避していたフックは、v2.1.77以降では意図通りに機能しません。設計を見直してください。
