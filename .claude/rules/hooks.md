...
>
> **v2.1.101 未認識イベント名の耐性向上**: v2.1.101以降、`settings.json` または `hooks.json` 内に未認識のイベント名が含まれていても、ファイル全体が無視されることはなくなりました。未認識のイベントのみスキップされ、他のフックは正常に動作します。これにより、新バージョンで追加されたイベントを古いバージョンのClaude Codeと共存させることが容易になります。
...
>
> **注意（v2.1.101以降）**: `permissionDecision: "ask"` を返しても、`permissions.deny` ルールが設定されている場合は **deny が優先** されます。以前は `ask` を返すことで deny を prompt に格下げできるバグがありましたが、v2.1.101で修正されました。
