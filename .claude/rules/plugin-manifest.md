...
> **注意**: `userConfig` の詳細スキーマ（必須フィールドなど）は公式ドキュメントを参照してください。
...
### allowManagedHooksOnly（v2.1.101以降で正常動作）

managed settings で `allowManagedHooksOnly` を設定すると、管理設定で許可されたフックのみ実行を許可するように制限できます。

> **v2.1.101修正**: v2.1.101以前では、managed settings で force-enable されたプラグインのフックが `allowManagedHooksOnly` 設定時に正しく動作しないバグがありました。v2.1.101以降は、force-enable されたプラグインのフックも `allowManagedHooksOnly` 設定下で正常に動作します。
...
- `allowManagedHooksOnly: true` の場合、ユーザーが手動で追加したフックはブロックされる
- managed settings で force-enable されたプラグインのフックは実行が許可される
- エンタープライズ環境でのフック実行ポリシーの一元管理に使用
