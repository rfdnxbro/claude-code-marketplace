...
- **`claude plugin enable` 実行時（v2.1.143以降）**: 宣言した依存プラグインがトランザクティブに強制有効化されます
- **`claude plugin disable` 実行時（v2.1.143以降）**: 他の有効なプラグインが対象プラグインに依存している場合、無効化が拒否されます（コピー可能な無効化チェーンのヒントが表示されます）
...
**settings.json の `deny` ルール（v2.1.166以降）:**

ユーザー側の `settings.json` では `deny: ["*"]` を指定することで全ツールを拒否できます。これはプラグインのfrontmatterフィールドではなく、settings.json の `deny` キーの動作変更です。
...
| `agent` | エージェント関連のデフォルト設定。**v2.1.157以降、dispatched sessions（サブエージェントとして呼び出されたセッション）でも反映される** | [agents.md](agents.md) および公式 [`/en/sub-agents`](https://code.claude.com/docs/en/sub-agents) を参照 |
