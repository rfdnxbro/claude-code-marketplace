...
**brace group（`{a,b,c}`のような展開表記）の制限:** `paths:` の値には `src/**/*.{ts,tsx}` のようなbrace group展開を使用できますが、brace group展開は予算（budget）内に制限されます。1つのパターンに大量のbrace groupを指定すると、展開が予算上限に達する場合があります（TODO: 要確認 — 予算の具体的な上限値、および上限超過時の挙動（無視されるかエラーになるか）は現時点では未確認）。
