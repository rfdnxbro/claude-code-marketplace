...
**ネストされたスキルディレクトリの自動検出**（v2.1.6以降）:

- サブディレクトリ内のファイルで作業する際、ネストされた `.claude/skills` ディレクトリからスキルを自動検出
- 作業ディレクトリの階層を遡り、すべての親ディレクトリの `.claude/skills` を検出
- プロジェクト構造に応じて柔軟にスキルを配置可能
...
上記の例で、`src/feature/`内で作業している場合、`skill-a`、`skill-b`、`skill-c`のすべてが利用可能になります。
