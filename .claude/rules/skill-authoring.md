---
paths: plugins/*/skills/**/SKILL.md, plugins/*/SKILL.md, .claude/skills/**/SKILL.md
---
...
フロントマターのフィールド値は文字列として扱われます。通常の英数字・日本語・記号（全角句読点、em ダッシュ、スラッシュ等）を含む値はクォート不要です。

明示的にクォートで囲むのは、YAML パーサーが値を意図と異なる型・構造に解釈し得るケースに限定します:

- 数値のみの値（例: `name: "3000"` — クォートなしでは数値型として解釈される）
- YAML フロー構文と衝突する値（例: `argument-hint: "[topic: foo | bar]"` — クォートなしでは sequence として解釈される）
...
# クォート不要（通常の文字列）
name: my-skill
description: 処理を実行する。ユーザーが Qiita/Zenn 記事を書く時に使用。
...
実害がない値（通常の日本語文・英数字混在の description 等）に対して一律にクォートを付ける必要はありません。既存ファイルへの整形目的の書き換えは churn を生むだけなので避けます。
...
- **呼び出し名として使用される**: `plugin.json` で `"skills": ["./"]` を使ってスキルを宣言した場合、スキルの呼び出し名には常にこの `name` フィールドが使用されます。インストール方法を問わず `name` フィールドが優先されます。

**description**:

- `/skills` メニューでの表示上限は1,536文字。1,536文字を超える description は起動時に Claude Code から truncation 警告が表示され、メニュー表示時に切り詰められます
- **本リポジトリのバリデーターは 1,536文字超をエラーとして検出します**（1,536文字以内に収めること）
- 三人称で記述（「ファイルを処理する」、「私がファイルを処理します」ではない）
- 何をするか AND いつ使うかを含める
- 発見性のため具体的なキーワードを含める
- **プラグイン名は自動表示されます**: スキルの説明と `/skills` メニューにプラグイン名が自動的に追加表示されるため、description内にプラグイン名を含める必要はありません
- **推奨**: 発見性向上のため `description` は常に記載することを推奨します。
...
使用可能な値: `sonnet`, `opus`, `haiku`

**effort**:

スキル実行時のモデルエフォートレベルをオーバーライドします。省略時は会話の設定を継承。
...
使用可能な値: `low`, `medium`, `high`, `xhigh`（対応モデルでのみ利用可能。非対応モデルでは `high` にフォールバック）, `max`
...
**注意（`Write`/`NotebookEdit`/`Glob`はパス指定が効かない）**: `Write(path)`・`NotebookEdit(path)`・`Glob(path)`のように括弧内にパスを指定しても、パス単位の絞り込みは機能しません（起動時に警告が表示されます）。特定のパスに対する操作を許可・制限したい場合は、代わりに`Edit(path)`や`Read(path)`を使用してください。

```yaml
# 悪い例: Write/NotebookEdit/Globはパス指定が効かない
allowed-tools:
  - Write(src/**)

# 良い例: パス単位の絞り込みが必要な場合はEdit/Readを使用
allowed-tools:
  - Edit(src/**)
```
...
```yaml
allowed-tools:
  - Agent(model:sonnet)   # Sonnetモデルのサブエージェントのみ許可
```

`disallowed-tools` と組み合わせて特定パラメータのツールを除外する例:

```yaml
disallowed-tools:
  - Agent(model:opus)     # Opusモデルのサブエージェントをブロック
```
...
Claude Codeは `/skills` メニューとスキル説明にプラグイン名を自動的に表示し、ユーザーがスキルの出所を簡単に識別できるようにします。
...
- descriptionにはスキルの機能と使用タイミングに集中する
- プラグイン名は自動表示されるため、description内に含める必要はない
- 具体的なキーワードを含めて、検索性と発見性を向上させる
- **`/skills` メニューでの表示上限は1,536文字**: 1,536文字以降は切り捨てられる。1,536文字を超えると起動時に警告が表示されます
...
# 良い description（機能と使用タイミングに集中）
description: OpenAPI仕様からAPIドキュメントを生成する。API定義ファイルがある場合に使用。
# 悪い description（プラグイン名を含めている）
# description: [api-tools] OpenAPI仕様からAPIドキュメントを生成する
---
```

## スキルの承認要件

スキルの実行時にユーザー承認が必要かどうかは、スキルが持つ権限やフックによって決まります。
...
| `${CLAUDE_SKILL_DIR}` | スキル自身のディレクトリへの絶対パス | スキルに隣接するリソースファイルの参照 |
| `${CLAUDE_EFFORT}` | 現在のエフォートレベル | エフォートに応じた処理分岐 |
...
`${CLAUDE_EFFORT}` を使用すると、現在のエフォートレベルに応じた処理を記述できます:
...
- medium: 標準的なレビュー
- high: 詳細な分析と改善提案
- xhigh: より深い分析（対応モデルでのみ利用可能。非対応モデルは high にフォールバック）
- max: 最大の思考予算で網羅的にレビュー
```

利用可能な値: `low`, `medium`, `high`, `xhigh`（対応モデルでのみ利用可能。非対応モデルでは `high` にフォールバック）, `max`

**対応する引数が渡されなかった場合の挙動**: 本文に `$1`、`$2` 等の位置指定プレースホルダーが含まれているが、実行時に対応する引数が渡されなかった場合、そのプレースホルダーはリテラル文字列としてそのまま出力に残ります（無言で削除されることはありません）。

**`\$` エスケープ構文**: スキルやコマンドのボディで `$1`、`$2` のような `$` + 数字の組み合わせをリテラルとして出力したい場合は `\$1`、`\$2` と記述します。これにより引数展開が行われず、リテラルの `$1` がそのまま表示されます。

```yaml
---
name: script-example
description: シェルスクリプトの書き方を案内する
---

シェルスクリプトで引数を使う例:
\$1 は最初の引数、\$2 は2番目の引数を表します。
```

`${CLAUDE_SKILL_DIR}` を使用すると、SKILL.mdと同じディレクトリにあるファイルを参照できます:
...
スキル本文内で `` !`command` `` 構文を使用すると、スキルがロードされる際にシェルコマンドを実行し、その出力をスキル内容に注入できます。

**構文:**

```markdown
現在の日時: !`date`
Gitブランチ: !`git branch --show-current`
```
...
- 環境固有の情報をスキルに動的に注入
- 設定ファイルの内容を埋め込む
- リポジトリの状態（ブランチ名、最新コミット等）を含める

**注意:**

- コマンドはスキルがコンテキストに読み込まれる際に実行される
- コマンドの実行時間が長い場合、スキルの読み込みが遅延する可能性がある
- セキュリティ上、信頼できるコマンドのみを使用すること
- `disableSkillShellExecution` 設定が有効な場合、`` !`command` `` 構文によるインラインシェル実行は無効化されます
...
Claude Codeは`.claude/skills`ディレクトリ内のスキルを自動的に検出します:
...
- **プラグインルートレベルのスキル**: `skills/` サブディレクトリがなくても、プラグインルートに `SKILL.md` を配置するだけでスキルとして認識される。`plugin.json` の `skills: ["./"]` と組み合わせて使用する
...
> **注意**: `plugin.json` の `skills:` フィールドにはディレクトリパスのみ指定可能です。ファイルパスを指定すると `claude plugin validate` がエラーを出力します。例: `"skills": ["./"]` や `"skills": ["./skills/"]` は有効ですが、`"skills": ["./my-skill.md"]` はエラーになります。
...
**文字数バジェット**: スキル一覧の説明文の合計はモデルのコンテキストウィンドウの1%に収まるよう調整されます。溢れると、利用頻度の低いスキルの説明から順に切り詰め・省略されます。バジェットは `skillListingBudgetFraction` 設定（例: `0.02` で2%）または `SLASH_COMMAND_TOOL_CHAR_BUDGET` 環境変数（固定文字数）で変更できます。なお各エントリの `description`＋`when_to_use` は、バジェットとは別に1エントリあたり1,536文字で上限が掛かります（`maxSkillDescriptionChars` 設定で変更可）。`/doctor` で切り詰め・省略の状況を確認できます。
...
サンドボックスモードで実行される場合、`.claude/skills`ディレクトリへの書き込みがブロックされます。
...
- スキルの動的生成が必要な場合は、プラグインのインストール時に静的に生成する
- 実行時の設定変更は、`.claude/skills`外のディレクトリに保存する
- `Setup`フック（`--init`または`--init-only`フラグ）を使用して初期セットアップ時にスキルを生成する
...
- サンドボックスモードで`.claude/skills`への書き込みを行う
