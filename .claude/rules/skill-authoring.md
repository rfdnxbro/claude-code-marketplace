...
- 最大1024文字
- **`/skills` メニューでの表示は250文字に制限されます**（v2.1.86以降）: 重要な情報は最初の250文字以内に収めることを推奨
...
> **注意（v2.1.101バグ修正）**: v2.1.101以前では、`agent` フィールドが実際には機能していないバグがありました。このフィールドを使用する場合は v2.1.101 以降を使用してください。
...
> **注意（v2.1.101バグ修正）**: v2.1.101以前では、`context: fork` が実際には機能していないバグがありました。このフィールドを使用する場合は v2.1.101 以降を使用してください。
...
- **重要な情報は最初の250文字以内に収める**（v2.1.86以降）: `/skills` メニューでは250文字以降は切り捨てられる
