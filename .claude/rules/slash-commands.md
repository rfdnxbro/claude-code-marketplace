...
- キーワード検出は単語境界を考慮したマッチングです。`dropdown`や`reproduction`のようにキーワードを部分文字列として含むだけの単語では警告は出ません。一方、`delete_all_records`や`production_env`のようなアンダースコア区切りの複合語や、`deletes`/`deployed`/`deploying`のような一般的な活用形（`s`/`es`/`d`/`ed`/`ing`）は引き続き検出されます。
