...
- キーワード検出は単語境界を考慮したマッチングです。`dropdown`や`reproduction`のようにキーワードを部分文字列として含むだけの単語では警告は出ません。一方、`delete_all_records`や`production_env`のようなアンダースコア区切りの複合語は単語境界の判定に含まれないため、引き続き検出されます。
