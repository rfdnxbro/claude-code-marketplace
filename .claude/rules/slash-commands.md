...
Claude Codeには組み込みのバンドルコマンドが用意されています。プラグイン開発者が定義するユーザー定義コマンドとは異なり、インストール不要で使用できます。

### 既知のバンドルコマンド一覧

| コマンド | 追加バージョン | 説明 |
|----------|--------------|------|
| `/simplify` | v2.1.63 | 変更されたコードを再利用性・品質・効率性の観点でレビューし、問題を修正 |
| `/batch` | v2.1.63 | 複数のタスクをバッチ処理で実行 |
| `/clear` | - | 会話をリセット（スキルキャッシュもリセット） |
| `/model` | - | 使用するモデルを切り替え |
| `/memory` | - | メモリの管理 |

**注意**: バンドルコマンドとユーザー定義コマンドで名前が衝突した場合の動作は保証されません。バンドルコマンドと同名のコマンドをプラグインで定義することは避けてください。

### `/clear` とスキルキャッシュ（v2.1.63で修正）

`/clear` コマンドは会話をリセットする際に、キャッシュされたスキルコンテンツもリセットします（v2.1.63で修正）。これにより、古いスキルキャッシュが新しい会話に引き継がれる問題が解消されました。
