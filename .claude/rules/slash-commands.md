...
| `\$1`, `\$2`... | 数字の前の `$` をリテラルとして出力（エスケープ構文・v2.1.163以降） | `\$1` → `$1`（展開されない） |
...
**`\$` エスケープ構文（v2.1.163以降）**: `$` の直後に数字が続く場合（`$1`, `$2` 等）、`\$` を使うと引数展開されずにリテラルの `$` として出力されます。シェルスクリプトのサンプルコードなど、`$1` をリテラルとして表示したい場合に使用します。
