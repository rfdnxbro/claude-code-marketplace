...
zipは配下に`.claude-plugin/`を直接含む構成、または単一のトップレベルフォルダの中に`.claude-plugin/`を含む構成のどちらでもインストール可能です。
...
| `url` | string | ✓ | zipアーカイブのHTTPS URL（`http://`は非対応） |
| `sha256` | string | | アーカイブのSHA-256ダイジェスト（16進数64文字、大文字小文字問わず） |

TODO: 要確認 — 以下は現時点で未確認。CHANGELOGは「zipアーカイブをHTTPS経由で（git/npm不要）インストールでき、オプションでSHA-256ピン留めが可能」とのみ述べており、それ以上の制約には触れていない。

- `.claude-plugin/` がより深い階層にネストされたzipがインストールできるか
- アーカイブサイズの上限があるか、あるとして何バイトか
- loopback / link-local / クラウドメタデータホストへのURLが拒否されるか、
  リダイレクト先にも同じ検証が適用されるか。**拒否されることを前提に
  信頼境界を設計しないこと**
- フィールド名が `sha256` で正しいか（CHANGELOGに明記がない）
