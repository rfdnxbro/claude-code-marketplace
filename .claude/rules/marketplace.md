...
| `renames` | object | プラグイン名変更マッピング（旧名 → 新名）。v2.1.193以降で自動適用 |

## プラグイン自動リネーム（v2.1.193以降）

`renames` フィールドを使用すると、プラグイン名を変更した際にユーザーの設定を自動的に更新できます。v2.1.193以降、マーケットプレイスの `renames` マップが自動的に適用され、古いプラグイン名を使用しているユーザーの設定が新しい名前に更新されます。
...
| `defaultEnabled` | boolean | `false` を指定するとマーケットプレイスからインストール時にデフォルト無効になる。`/plugin` または `claude plugin enable` で有効化できる。有効化済みプラグインの依存として指定された場合は自動的に有効化される（v2.1.154以降） |
...
| `skipLfs` | boolean | | `true` に設定すると Git LFS のダウンロードをスキップ（v2.1.153以降） |
...
> **注意（v2.1.81以降）**: branch/tagを指定したプラグイン（ref-tracked plugins）は、**毎回ロード時に再クローン**されて最新版に更新されます。特定のバージョンに固定したい場合はcommit SHAを指定してください。
>
...
`settings.json` 内にインラインでプラグインエントリを宣言できるソース種別です。マーケットプレイスファイルを別途用意せずに、Claude Code の設定ファイルに直接プラグインを埋め込む場合に使用します（v2.1.80以降）:
...
## シードディレクトリの複数指定（v2.1.79以降）

`CLAUDE_CODE_PLUGIN_SEED_DIR` 環境変数を使用してプラグインのシードディレクトリを指定できます。v2.1.79以降、複数のディレクトリをプラットフォームのパス区切り文字で区切って指定できるようになりました。
...
## `--plugin-dir` での `.zip` アーカイブ指定（v2.1.128以降）

v2.1.128以降、`--plugin-dir` はディレクトリだけでなく `.zip` アーカイブも受け入れます。

```bash
# ディレクトリを指定（従来通り）
claude --plugin-dir ./my-plugin

# .zip アーカイブを指定（v2.1.128以降）
