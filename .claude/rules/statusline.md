...
| `refreshInterval` | No | N秒ごとにステータスラインコマンドを再実行する間隔（秒） |
...
| `workspace.added_dirs` | array | `/add-dir` で追加したディレクトリ一覧 | `["/extra/dir"]` |
| `workspace.git_worktree` | string | 現在のディレクトリがリンクされたgit worktree内にある場合に設定される。それ以外は存在しない | `/path/to/linked/worktree` |
...
> **注意**: `workspace.git_worktree` とは異なります。`worktree` は `--worktree` フラグで起動したセッション専用のフィールドで、セッション管理情報を含みます。`workspace.git_worktree` は通常セッションでも現在のディレクトリがリンクされたgit worktree内にある場合に設定されます。
...
GitHubリポジトリとPRが検出された場合のみ存在するフィールドです。検出されない場合は各フィールドが省略されます。

| フィールド | 型 | 説明 | 例 |
|-----------|---|------|-----|
| `github_repo` | object? | GitHubリポジトリ情報（検出時のみ）TODO: 要確認 - 正確なサブフィールドは未調査 | - |
| `github_pr` | object? | GitHub PR情報（検出時のみ）TODO: 要確認 - 正確なサブフィールドは未調査 | - |
